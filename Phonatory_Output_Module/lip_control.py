...
class LipController:
    ...
    def __init__(self, config=None, articulation_reference=None):
        ...
        # Use seed data to bias lip behavior
        if self.articulation_reference:
            ...
            if 'precision' in symbolic_tone:
                self.lip_rounding = 0.3  # Moderate rounding for precision
            else:
                self.lip_rounding = 0.0
        
        # Lip position parameters (placeholder)
        self.lip_rounding = getattr(self, 'lip_rounding', 0.0)      # 0 = spread, 1 = rounded
