...
class SetupProjectToolTest {
    ...
    @Test
    fun `execute should NOT create UCM-specific files`() = runBlocking {
        val params = JsonObject(emptyMap())
        ...
        // Verify .ucm directory does NOT exist (user-managed separately if using UCM)
        val ucmDir = workingDir.resolve(".ucm")
        assertFalse(Files.exists(ucmDir), ".ucm directory should NOT exist (user-managed, not created by setup_project)")

        // Verify response does NOT include UCM-specific fields
        val data = responseObj["data"]?.jsonObject
        assertNull(data!!["ucmDirectoryCreated"], "Should NOT have ucmDirectoryCreated field")
