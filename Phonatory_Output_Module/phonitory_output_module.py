...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'Main_Core'))
...
class PhonatoryOutputModule:
    ...
    def __init__(self, config_path="voice_config.json"):
        ...
        # Load articulation reference from seed
        self.articulation_reference = get_seed()
        self.logger.info("Articulation reference loaded from seed")
        # Pass config to advanced modules if available
        self.larynx = LarynxSimulator(config.get("larynx_params", {}), self.articulation_reference)
        self.formant = FormantFilter(config.get("formant_params", {}))
        self.tongue = TongueArticulator(config.get("tongue_params", {}), self.articulation_reference)
        self.lip = LipController(config.get("lip_params", {}), self.articulation_reference)
    ...
    def phonate(self, text: str, out_path=None, pitch_factor=1.0, formant_target=None, articulation=None, nasalization=None):
        ...
        if out_path is None:
            out_path = f"output_voice_{ISS.pulse().replace('PULSE-', '').replace('.', '_')}.wav"
