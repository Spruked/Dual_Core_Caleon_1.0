...
- AI-specific formats: UCM.md, .cursorrules, .github/copilot-instructions.md, .windsurfrules
...
- AI-specific formats: UCM.local.md, .cursor/local, .ai/local.config
...
### Step 0.2: Detect UCM Usage

**Check if the user is using UCM** (enables optional advanced features):

**Detection methods** (try in order):
1. Check for UCM integration in project (look for UCM references in config files)
2. Ask user: "Are you using UCM for this project? (enables advanced AI orchestration) [Y/n]"

**Based on detection**:
- **If UCM detected/confirmed**: Continue to Step 0.3
- **If NOT UCM**: Skip to Step 1 (basic setup complete)

### Step 0.3: Run setup_ucm_orchestration Tool (UCM Only)

**If UCM is being used**, execute the setup_ucm_orchestration tool:

```
setup_ucm_orchestration()
```

**What this creates** (UCM specific):
- `ucm/` directory with AI orchestration skills
- UCM integration configuration for Task Orchestrator
- Advanced AI reasoning and voice articulation capabilities
...
**Note**: If user is not using UCM, this step is skipped and they use Task Orchestrator via direct tool calls.
...
[If UCM:]
- UCM integration enabled
- Advanced AI orchestration available in ucm/
...
- Does project documentation exist? (UCM.md, .cursorrules, .github/copilot-instructions.md, .ai/PROJECT.md)
...
**Check your memory** (UCM.md, .cursorrules, etc.) for "Task Orchestrator - AI Initialization" section:
...
- UCM.md, .cursorrules, .github/copilot-instructions.md, or similar
...
Your existing documentation remains the source of truth in [UCM.md/etc].
