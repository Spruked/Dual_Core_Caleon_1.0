...
**Detection Steps**:
1. Check if your AI's standard documentation file exists (UCM.md, .cursorrules, etc.)
...
3. Check for UCM integration in project
4. Determine what's NEW and available:

**Feature Detection Logic**:
```
If UCM integration detected:
    Check Features field for "ucm-integration"
    ├─ Missing "ucm-integration" → Offer UCM integration installation
    └─ Has "ucm-integration" → Skip (already installed)

    Check Features field for "cerebral-cortex"
    ├─ Missing "cerebral-cortex" → Offer cerebral cortex integration
    └─ Has "cerebral-cortex" → Skip (already installed)

    Check if ucm/ directory exists with skills
    ├─ Not found → Offer UCM skill installation
    └─ Found → Confirm already installed

    Check if UCM cerebral cortex is connected
    ├─ Not connected → Offer connection setup
    └─ Connected → Confirm already installed
Else:
    Report: "UCM not detected. Advanced features require UCM integration."
```
...
**System 2: UCM Cerebral Cortex Orchestration (Recommended)**:
- AI-powered reasoning and orchestration through Phi-3 Mini LLM
- Works with UCM integration (requires cerebral cortex connection)
- Setup required: Run `setup_ucm_orchestration` tool first
- Ethical oversight and consent management via Caleon
- Voice articulation capabilities for enhanced interaction
- Use for: Complex features, ethical AI decision-making, voice-enabled workflows
...
### Step 7: UCM Integration Features (Recommended)

**Recommended for advanced AI orchestration** - Check for UCM integration in project.

If UCM integration detected, offer advanced UCM features using AskUserQuestion:

```
AskUserQuestion(
  questions: [{
    question: "UCM integration detected! Which advanced features would you like?",
    header: "Features",
    multiSelect: false,
    options: [
      {
...
**Option [1] or [3]: Install UCM Integration**

If user chooses UCM integration:
1. Check if UCM cerebral cortex is available
2. If available: Establish connection to UCM
3. Configure Task Orchestrator for UCM orchestration:
   ```json
   {
     "ucm_integration": {
       "cerebral_cortex_endpoint": "http://localhost:8080",
       "phi_3_mini_enabled": true,
       "caleon_consent_enabled": true,
       "voice_articulation": "vallm",
   ...
   ```
4. Establish UCM cerebral cortex connection
5. Verify UCM integration is working

**Option [2] or [3]: Install UCM Skills and Orchestration**

If user chooses UCM orchestration:
1. Run the `setup_ucm_orchestration` tool
2. This creates `ucm/` directory with AI orchestration skills:
   ...
   - ucm-integration.md
3. This establishes UCM cerebral cortex integration with:
   - Phi-3 Mini LLM reasoning capabilities
   - Caleon ethical oversight and consent management
   - VALLM voice articulation for responses
   - Symbolic memory vault for context retention
...
[If UCM integration installed:]
🤖 UCM Orchestration Active:
   • Cerebral cortex reasoning enabled
   • Ethical oversight and consent management active
   • Voice articulation capabilities available
   • Configuration: UCM cerebral cortex connection

   💡 To modify: Update UCM integration settings

[If UCM orchestration installed:]
🤖 UCM Skills & Integration Ready:
   • AI reasoning and orchestration through cerebral cortex
   • Ethical decision-making with Caleon consent management
   • Voice-enabled responses via VALLM
   • Skill definitions: ucm/*.md
   • See UCM documentation for complete integration guide
...
- Detects UCM integration → Optionally calls `setup_ucm_orchestration`
