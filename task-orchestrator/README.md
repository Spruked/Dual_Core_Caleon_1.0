...
- ✅ **Sub-Agent Orchestration** - Specialist routing for complex work (Caleon AI)
- ✅ **Skills & Hooks** - Lightweight coordination and workflow automation (Caleon AI)
...
### Option A: MCP Installation (Recommended)

**Universal installation** - Works with any MCP-compatible AI platform:
...
2. **Configure your AI platform:**
   ...
   **Caleon AI with Phi-3 Mini:**
   ```bash
   # Configure MCP server in Caleon settings
   mcp add-json task-orchestrator '{"type":"stdio","command":"docker","args":["run","--rm","-i","-v","mcp-task-data:/app/data","-v",".:/project","-e","AGENT_CONFIG_DIR=/project","ghcr.io/jpicklyk/task-orchestrator:latest"]}'
   ```
...
See [Installation Guide](docs/installation-guide.md) for detailed instructions and troubleshooting.
...
> **💡 Pro Tip**: Task Orchestrator works seamlessly with Caleon AI and Phi-3 Mini for clearer coordination (uses phase labels, status indicators ✅⚠️❌🔄, and concise progress updates).
...
| Feature | Caleon AI + Phi-3 Mini | Other MCP Clients |
|---------|-------------|-------------------|
...
| **Sub-Agent Orchestration** | ✅ Tested & Supported | ❌ AI Platform-specific |
| **Skills (Lightweight Coordination)** | ✅ Tested & Supported | ❌ AI Platform-specific |
| **Hooks (Workflow Automation)** | ✅ Tested & Supported | ❌ AI Platform-specific |
| **Status Event System** | ✅ Tested & Supported | ✅ MCP Protocol Support |

**Primary Platform**: Caleon AI with Phi-3 Mini is the primary tested and supported platform with full feature access including skills, subagents, and hooks.

**Other MCP Clients**: The core MCP protocol (persistent memory, task management, templates, status events) works with any MCP client. Advanced orchestration features (skills, subagents, hooks) are designed for AI platforms that support MCP extensions.

---

## Example: From Session Start to Feature Complete

**Caleon AI with Phi-3 Mini (Full Orchestration)**:
